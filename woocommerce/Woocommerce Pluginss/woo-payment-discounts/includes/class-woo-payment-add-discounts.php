...
class Woo_Payment_Add_Discounts {
    ...
    protected function discount_name($value, $gateway) {
        if (strstr($value, '%')) {
            return sprintf(__('Desconto por %s (%s off)', 'woo-payment-discounts'), esc_attr($gateway->title), $value);
        }
        return sprintf(__('Desconto por %s', 'woo-payment-discounts'), esc_attr($gateway->title));
    }
    ...
    public function wpd_payment_method_title($title, $id) {
        ...
        if (isset($settings[$id]['amount']) && 0 < $settings[$id]['amount']) {
            ...
            $title .= ' <small>(' . sprintf(__('%s de desconto', 'woo-payment-discounts'), $value) . ')</small>';
        }
        return $title;
    }
