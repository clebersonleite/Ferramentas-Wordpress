...
class WC_Braspress {
	...
	public function add_cpf_cnpj_field( array $fields ){
        ...
        $is_cpf_cnpj_required = (int) $wpdb->get_var(
            "Select 
                count(*) 
            From 
                wp_options  
            where 
                option_name like 'woocommerce_braspress-%_settings' 
                AND 
                option_value like '%s:7:\"enabled\";s:3:\"yes\"%s:16:\"require_cpf_cnpj\";s:3:\"yes\"%'"
        );
        ...
        $fields['billing']['billing_cpf_cnpj'] = array(
            ...
            'priority'      => 1,
            'maxlength'     => "18",
            'required'      => $is_cpf_cnpj_required,
            'placeholder'   => 'CPF / CNPJ',
            'class'         => ($is_cpf_cnpj_required)
                ? array ('address-field', 'update_totals_on_change', 'cpf_cnpj' )
                : array ('address-field', 'update_totals_on_change', 'cpf_cnpj', 'hidden' ),
