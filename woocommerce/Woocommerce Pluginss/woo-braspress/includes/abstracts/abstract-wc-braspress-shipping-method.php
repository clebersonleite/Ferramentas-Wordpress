...
abstract class WC_Braspress_Shipping_Method extends WC_Shipping_Method {
     ...
     * @var mixed
     */
    private $is_the_cpf_cnpj_required;
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_description   = sprintf(__( "Este é um método de envio da %s", 'wc-braspress' ), $this->method_title );
		...
		$this->is_the_cpf_cnpj_required = $this->get_option( 'require_cpf_cnpj' );;
	...
	public function get_destination_cpf_cnpj(): string {
	    ...
	    if ($this->is_the_cpf_cnpj_required()) {
	        ...
	        $this->set_destination_cpf_cnpj($this->origin_cpf_cnpj);
        }
	...
	public function init_form_fields() {
		$this->instance_form_fields = array(
			'enabled' => array(
				'title'   => __( 'Habilitar/Desabilitar', 'wc-braspress' ),
				'type'    => 'checkbox',
				'label'   => __( 'Habilitar este método de envio', 'wc-braspress' ),
				'default' => 'yes',
			),
			'require_cpf_cnpj' => array(
				'title'   => __( 'CPF/CNPJ do cliente', 'wc-braspress' ),
				'type'    => 'checkbox',
				'label'   => __( 'Habilita/Desabilita a solicitação do CPF/CNPJ ao cliente na simulação do frete', 'wc-braspress' ),
				'default' => 'yes',
			),
			'title' => array(
				'title'       => __( 'Title', 'wc-braspress' ),
				'type'        => 'text',
				'description' => __( 'Titulo que será exibido na tela de finalizar o pedido.', 'wc-braspress' ),
				'desc_tip'    => true,
				'default'     => $this->method_title,
			),
			'behavior_options' => array(
				'title'   => __( 'Opções de comportamento', 'wc-braspress' ),
				'type'    => 'title',
				'default' => '',
			),
			'show_deadline_days' => array(
				'title'   => __( 'Exibir/Ocultar', 'wc-braspress' ),
				'type'    => 'checkbox',
				'label'   => __( 'Exibe a estimativa de entrega.', 'wc-braspress' ),
				'default' => 'yes',
			),
			'additional_days' => array(
				'title'            => __( 'Dias adicionais.', 'wc-braspress' ),
				'type'             => 'text',
				'description'      => __( 'Incrementa o prazo de entrega ao informado pela Braspress.', 'wc-braspress' ),
				'desc_tip'         => true,
				'default'          => '0',
				'placeholder'      => '0',
			),
			'shipping_class_id' => array(
				'title'       => __( 'Classe de entrega.', 'wc-braspress' ),
				'type'        => 'select',
				'description' => __( 'Se necessário, selecione a classe de entrega para aplicar este método.', 'wc-braspress' ),
				'desc_tip'    => true,
				'default'     => '',
			...
			'origin_postcode' => array(
				'title'       => __( 'CEP de origem', 'wc-braspress' ),
				'type'        => 'text',
				'description' => __( 'O CEP do local onde as suas encomendas serão enviadas', 'wc-braspress' ),
			...
			'cnpj_origin' => array(
				'title'            => __( 'CNPJ de origem', 'wc-braspress' ) ,
				'type'             => 'text',
				'description'      => __( 'O CNPJ da empresa realizará a venda.', 'wc-braspress' ),
			...
			$this->instance_form_fields = array_merge(
				...
				array(
					'optional_services' => array(
						'title'         => __( 'Serviços opcionais', 'wc-braspress' ),
						'type'          => 'title',
						'default'       => '',
					),
					'fob_message'       => array(
						'title'         => __( 'Mensagem FOB (Retirada na Braspress)', 'wc-braspress' ),
						'type'          => 'text',
						'label'         => __( 'Habilitar', 'wc-braspress' ),
						'description'   => __( 'Mensagem que será exibida quando a forma de envio FOB for selecionada.', 'wc-braspress' ),
						'desc_tip'      => true,
						'default'       => __( 'Retirar na Braspress', 'wc-braspress' )
					)
				)
			);
		}
	...
	protected function get_shipping_classes_options() {
		...
		$options          = array(
			'-1' => __( 'Qualquer classe de envio',  'wc-braspress' ),
			'0'  => __( 'Nenhuma classe de envio',    'wc-braspress' ),
		);
     ...
     */function get_estimating_delivery( $title, $deadline_days, $additional_days = 0 ) {
		...
		if ( $total_additional_days > 0 ) {
			$title .= ' (' . sprintf( _n( '%d dia útil', '%d dias úteis', $total_additional_days, 'wc-braspress' ),  $total_additional_days ) . ')';
		}
     ...
     * @return bool
     */
    private function is_the_cpf_cnpj_required() {

	    return ($this->is_the_cpf_cnpj_required === 'yes');
    }
